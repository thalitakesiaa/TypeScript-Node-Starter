...
describe("Teste de Encontrar Perfil pelo Email", () => {
  ...
  beforeAll(async () => {
    ...
    await typeorm.createConnection({ ...connectionOptions, name: "testConnection" });
    profileRepository = typeorm.getConnection("testConnection").getCustomRepository(ProfileRepository);
  });

  afterAll(async () => {
    const connection = typeorm.getConnection("testConnection");
